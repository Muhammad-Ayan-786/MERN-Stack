...
const UserHomePage = () => {
  ...
  return (
    <section className="flex min-h-0 flex-col gap-4 pb-2 sm:gap-6">
      ...
      <div className="grid grid-cols-1 gap-3 sm:gap-4 lg:min-h-0 lg:flex-1 lg:grid-cols-2 lg:items-stretch lg:gap-6">

        {/* Albums column */}
        <div className="flex min-h-[240px] flex-col rounded-2xl border border-white/10 bg-white/4 p-3 backdrop-blur-sm sm:p-4 lg:min-h-0">
          <h2 className="mb-3 shrink-0 text-sm font-semibold uppercase tracking-wider text-zinc-400">
            Albums
          </h2>
          <div className="albums scrollbar-adaptive flex-1 pr-1 lg:min-h-0 lg:overflow-y-auto lg:overscroll-contain">
            {
              albumsObj.isLoading
                ?
                ...
                <div className="grid grid-cols-2 gap-2 min-[480px]:grid-cols-3 sm:gap-3 lg:grid-cols-2 xl:grid-cols-3">
                  {albumsObj.data.map((album, idx) => (
                    <div
                      key={idx}
                      className="album relative flex aspect-4/5 min-h-[110px] flex-col justify-end overflow-hidden rounded-xl border border-white/10 bg-linear-to-br from-emerald-900/80 to-zinc-900/90 p-2.5 shadow-[0_8px_24px_-10px_rgba(74,222,128,0.35)] sm:aspect-square sm:max-h-36 sm:p-2"
                    >
